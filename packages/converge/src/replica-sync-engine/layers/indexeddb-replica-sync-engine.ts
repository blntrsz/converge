 ...
 * @category constants
 */
export const defaultPullInterval = Duration.minutes(1);
...
> =>
  Layer.effect(
    ReplicaSyncEngine,
    Effect.gen(function* () {
      const pullInterval = options.pullInterval ?? defaultPullInterval;
      ...
      const findAcceptedEventRow = (eventId: string) =>
        eventHistory
          .select("eventId")
          .equals(eventId)
          .limit(1)
          .pipe(
            Effect.map((rows) => Option.fromUndefinedOr(rows[0] as EventHistoryRow | undefined)),
            Effect.orDie,
          );

      const findAcceptedEvent = (eventId: string) =>
        findAcceptedEventRow(eventId).pipe(Effect.map(Option.map(eventFromRow)));
      ...
      const deleteAcceptedEvent = (eventId: string) =>
        eventHistory
          .delete()
          .limit(1)
      ...
      const applyLocally = (
        event: EventInstance,
        options: { readonly moveExistingToTail?: boolean } = {},
      ) =>
        acceptLock
          .withPermits(1)(
            Effect.gen(function* () {
              ...
              if (Option.isSome(existing)) {
                if (options.moveExistingToTail) {
                  yield* deleteAcceptedEvent(event.eventId);
      ...
      const pullRemoteEventsFrom = (initialCursorOption: Option.Option<string>) =>
        Effect.gen(function* () {
          let cursorOption = initialCursorOption;
          while (true) {
            const page = yield* primary.pull(Option.getOrUndefined(cursorOption));
            for (const event of page.data) {
              yield* applyLocally(event, { moveExistingToTail: true });
            }
            if (page.hasNext) {
      ...
      const pullRemoteEvents = Effect.gen(function* () {
        ...
        const cursorOption = yield* lastEventId();
        yield* pullRemoteEventsFrom(cursorOption);
      });

      const process = (task: Task) =>
        Effect.gen(function* () {
          const cursorBeforeFlush = yield* lastEventId();
          yield* flushProposedEvents;
          if (task.kind === "reconcile") {
            yield* pullRemoteEvents;
          } else {
            yield* pullRemoteEventsFrom(cursorBeforeFlush);
          }
        ...
        function* () {
          ...
          if (mode._tag === "Checkout") {
            ...
            if (Option.isNone(event)) {
              return yield* Effect.die(
                new Error(`Cannot repair at unknown eventId ${mode.eventId}`),
              );
            }
      ...
      const enqueueReconcile = Effect.gen(function* () {
      ...
      const poll = Effect.gen(function* () {
        while (true) {
          yield* Effect.sleep(pullInterval);
          const mode = yield* Ref.get(syncMode);
          if (mode._tag === "Latest") {
            yield* enqueueReconcile;
          }
