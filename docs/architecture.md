...
Primary and replica handlers are separate implementations wired through `EventRouter`. They must produce the same projection snapshot for the same event sequence. Shared logic lives in application-level reduce functions (see `examples/react/core`).
