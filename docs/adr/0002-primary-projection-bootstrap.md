...
Replicas cold-start by pulling a projection snapshot and event history id from the primary, then continue syncing incrementally from the event log. The primary maintains its own persisted projections (same abstraction as replicas) and exposes them over the sync API. Replicas never replay the full event history locally.
